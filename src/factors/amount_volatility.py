...
@register_factor("large_order_volatility")
class LargeOrderVolatilityFactor(BaseFactor):
    """
    超大单成交额波动率因子

    计算过去N日超大单成交额的标准差，衡量大资金活跃度的波动性
    """

    def __init__(self,
                 name: str = "large_order_volatility",
                 window: int = 20,
                 preprocess: bool = True,
                 standardize_method: Optional[Literal['zscore', 'rank', 'minmax']] = 'zscore',
                 winsorize: Optional[Union[tuple[float, float], Literal['mad']]] = 'mad',
    ...
    def compute(self, config: FactorConfig) -> pd.DataFrame:
        ...
        # 获取超大单成交额数据
        loader = get_loader()
        large_order_amounts = loader.get_industry_large_order_amount(extended_start, config.end_date)

        # 计算滚动标准差
        large_order_vol = large_order_amounts.rolling(
            window=self.window,
            min_periods=int(self.window * 0.8)
        ).std()
        ...
        # 筛选到调仓日
        rebalance_dates = config.get_rebalance_dates()
        result = -large_order_vol.reindex(rebalance_dates)

        logger.info(f"超大单成交额波动因子计算完成，形状: {result.shape}")
        return result
...
if __name__ == '__main__':
    ...
    # 计算因子
    factor = LargeOrderVolatilityFactor(window=20)
    main_amount_vol = factor(test_config)

    print("\n成交波动因子（最近5期）：")
    print(main_amount_vol.tail())

    print(f"\n调仓日数量: {len(main_amount_vol)}")